			...
			<div class="container">
				...
				<div class="col-sm-3 col-md-3" id="block-menu-content">
						<ul class="block-menu" data-spy="affix" data-offset-top="500" data-offset-bottom="400">
							...
							<li><a class="faq-button" href="pagination?lastNum=1"><i class="icon fa fa-th-list"></i> 자유 게시판</a></li>
							<li><a class="faq-button active" href="qapagination?lastNum=1"><i class="icon fa fa-picture-o"></i> Q&A</a></li>
						</ul>
					</div>
							  ...
							  <tbody id="plus" name="plus">
								<c:forEach items="${list}" var="i" varStatus="status">
									<tr>
										...
										<td><a href="qadetail/${i.qaBoardNo}">${i.qaTitle}</a></td>
										<td class="hidden-xs" id="email">${i.email}</td>
										<td class="hidden-xs hidden-sm">${i.qaDate}</td>
										<td class="hidden-xs">${i.qaHits}</td>
										<td id="aa">
												<a href="#" id="defaultA">
													<i id="default_btn" class="icon fa fa-cog" ></i>
												</a>
											<span id="modify_delete_dis" style="width:100%; height:100%; display:none">
												<a href="#" id="modify_update_a" name="${i.qaBoardNo}">
													<i id="modify_btn" class="icon fa fa-wrench"></i>
												</a>
<!-- 아이디 들어오면 권한 비교하는거 해야되 -->	<a href="#" id="modify_delete_a" name="${i.qaBoardNo}">
	...
	$(document).on("mouseenter" , "#default_btn", function(){
 	...
 	$("#insert").click(function(){
 		if(login_id==""){
 			...
 			location.href="qaInsert_freeboard/editor";
 		}
	});
 	 ...
 	 $("#pagination ul li a").click(function(){
		...
		location.href="qapagination?lastNum="+num;
 	});
 		...
 		$.ajax({
 			...
 			success: function(result){
 				if(result.confirm=="false"){
 					...
 					location.href="qamodify_freeboard?modifyNum="+result.updateIcon;
 				}
 		...
 		$.ajax({
 			...
 			success: function(result){
 				if(result.confirm=="false"){
 					...
 					location.href="qadelete/"+result.updateIcon;
 				}
 				/* alert(result.updateIcon);
 				alert(result.confirm); */
